...
//                work.
/* solve these error by by deceiving the compiler through annotating some constexpr
/home/zsy/posix_quic-master/libquic/base/time/time.h:203:20: error: assignment of member ‘base::TimeDelta::delta_’ in read-only object
     delta_ = other.delta_;
                    ^~~~~~
/home/zsy/posix_quic-master/libquic/base/time/time.h:204:12: error: binding ‘const base::TimeDelta’ to reference of type ‘base::TimeDelta&’ discards qualifiers
     return *this;
            ^~~~~
/home/zsy/posix_quic-master/libquic/base/time/time.h:205:3: error: body of constexpr function ‘constexpr base::TimeDelta& base::TimeDelta::operator=(base::TimeDelta) const’ not a return-statement
   }
   ^
/home/zsy/posix_quic-master/libquic/base/time/time.h: In static member function ‘static constexpr base::TimeDelta base::TimeDelta::FromProduct(int64_t, int64_t)’:
/home/zsy/posix_quic-master/libquic/base/time/time.h:803:1: error: body of constexpr function ‘static constexpr base::TimeDelta base::TimeDelta::FromProduct(int64_t, int64_t)’ not a return-statement
 }
*/
...
#endif
//constexpr
#define my_const_expr constexpr
#define my_const_null 
namespace base {
...
class BASE_EXPORT TimeDelta {
 public:
  my_const_expr TimeDelta() : delta_(0) {}

  // Converts units of time to TimeDeltas.
  static my_const_expr TimeDelta FromDays(int days);
  static my_const_expr TimeDelta FromHours(int hours);
  static my_const_expr TimeDelta FromMinutes(int minutes);
  static my_const_expr TimeDelta FromSeconds(int64_t secs);
  static my_const_expr TimeDelta FromMilliseconds(int64_t ms);
  static my_const_expr TimeDelta FromMicroseconds(int64_t us);
  static my_const_expr TimeDelta FromNanoseconds(int64_t ns);
  static my_const_expr TimeDelta FromSecondsD(double secs);
  static my_const_expr TimeDelta FromMillisecondsD(double ms);
  static my_const_expr TimeDelta FromMicrosecondsD(double us);
  static my_const_expr TimeDelta FromNanosecondsD(double ns);
#if defined(OS_POSIX)
  ...
  static my_const_expr TimeDelta FromInternalValue(int64_t delta) {
  ...
  static my_const_expr TimeDelta Max();
  ...
  static my_const_expr TimeDelta Min();
  ...
  //
  // DEPRECATED - Do not use in new code. http://crbug.com/634507
  my_const_expr int64_t ToInternalValue() const { return delta_; }

  // Returns the magnitude (absolute value) of this TimeDelta.
  /*my_const_expr*/ TimeDelta magnitude() const {
  ...
  my_const_expr bool is_zero() const { return delta_ == 0; }

  // Returns true if the time delta is the maximum/minimum time delta.
  my_const_expr bool is_max() const {
    return delta_ == std::numeric_limits<int64_t>::max();
  }
  my_const_expr bool is_min() const {
  ...
  /*my_const_expr*/ TimeDelta& operator=(TimeDelta other) {
    delta_ = other.delta_;
    return *this;
  }

  // Computations with other deltas. Can easily be made my_const_expr with C++17 but
  ...
  my_const_expr TimeDelta operator-() const { return TimeDelta(-delta_); }

  // Computations with numeric types. operator*() isn't my_const_expr because of a
  ...
  template <typename T>
  my_const_expr TimeDelta operator/(T a) const {
    CheckedNumeric<int64_t> rv(delta_);
    rv /= a;
    if (rv.IsValid())
      return TimeDelta(rv.ValueOrDie());
  ...
  template <typename T>
  my_const_expr TimeDelta& operator/=(T a) {
    return *this = (*this / a);
  }

  my_const_expr int64_t operator/(TimeDelta a) const { return delta_ / a.delta_; }
  my_const_expr TimeDelta operator%(TimeDelta a) const {
    return TimeDelta(delta_ % a.delta_);
  }

  // Comparison operators.
  my_const_expr bool operator==(TimeDelta other) const {
  ...
  my_const_expr TimeDelta(const TimeDelta& other) : delta_(other.delta_) {}
#endif
 ...
 private:
  ...
  my_const_expr explicit TimeDelta(int64_t delta_us) : delta_(delta_us) {}

  // Private method to build a delta from a double.
  static my_const_expr TimeDelta FromDouble(double value);

  // Private method to build a delta from the product of a user-provided value
  // and a known-positive value.
  static my_const_expr TimeDelta FromProduct(int64_t value, int64_t positive_value);
...
class TimeBase {
 ...
 protected:
  my_const_expr explicit TimeBase(int64_t us) : us_(us) {}
...
class BASE_EXPORT Time : public time_internal::TimeBase<Time> {
 public:
  ...
  // 1800, and 1900.
  static my_const_expr int64_t kTimeTToMicrosecondsOffset =
  ...
  static my_const_expr int64_t kQPCOverflowThreshold = INT64_C(0x8637BD05AF7);
#endif
...
// limit year.
#if defined(OS_WIN)
  static my_const_expr int kExplodedMinYear = 1601;
  static my_const_expr int kExplodedMaxYear = 30827;
#elif defined(OS_IOS)
  static my_const_expr int kExplodedMinYear = std::numeric_limits<int>::min();
  static my_const_expr int kExplodedMaxYear = std::numeric_limits<int>::max();
#elif defined(OS_MACOSX)
  static my_const_expr int kExplodedMinYear = 1902;
  static my_const_expr int kExplodedMaxYear = std::numeric_limits<int>::max();
  ...
  static my_const_expr int kExplodedMaxYear = std::numeric_limits<int>::max();
#else
  static my_const_expr int kExplodedMinYear =
      (sizeof(time_t) == 4 ? 1902 : std::numeric_limits<int>::min());
  static my_const_expr int kExplodedMaxYear =
...
// static
my_const_expr TimeDelta TimeDelta::FromDays(int days) {
...
// static
my_const_expr TimeDelta TimeDelta::FromHours(int hours) {
...
// static
my_const_expr TimeDelta TimeDelta::FromMinutes(int minutes) {
...
// static
my_const_expr TimeDelta TimeDelta::FromSeconds(int64_t secs) {
  return FromProduct(secs, Time::kMicrosecondsPerSecond);
}

// static
my_const_expr TimeDelta TimeDelta::FromMilliseconds(int64_t ms) {
  return FromProduct(ms, Time::kMicrosecondsPerMillisecond);
}

// static
my_const_expr TimeDelta TimeDelta::FromMicroseconds(int64_t us) {
  return TimeDelta(us);
}

// static
my_const_expr TimeDelta TimeDelta::FromNanoseconds(int64_t ns) {
  return TimeDelta(ns / Time::kNanosecondsPerMicrosecond);
}

// static
my_const_expr TimeDelta TimeDelta::FromSecondsD(double secs) {
  return FromDouble(secs * Time::kMicrosecondsPerSecond);
}

// static
my_const_expr TimeDelta TimeDelta::FromMillisecondsD(double ms) {
  return FromDouble(ms * Time::kMicrosecondsPerMillisecond);
}

// static
my_const_expr TimeDelta TimeDelta::FromMicrosecondsD(double us) {
  return FromDouble(us);
}

// static
my_const_expr TimeDelta TimeDelta::FromNanosecondsD(double ns) {
  return FromDouble(ns / Time::kNanosecondsPerMicrosecond);
}

// static
my_const_expr TimeDelta TimeDelta::Max() {
  return TimeDelta(std::numeric_limits<int64_t>::max());
}

// static
my_const_expr TimeDelta TimeDelta::Min() {
  return TimeDelta(std::numeric_limits<int64_t>::min());
}

// static
my_const_expr TimeDelta TimeDelta::FromDouble(double value) {
...
// static
my_const_expr TimeDelta TimeDelta::FromProduct(int64_t value,
...
class BASE_EXPORT TimeTicks : public time_internal::TimeBase<TimeTicks> {
 public:
  ...
  my_const_expr TimeTicks() : TimeBase(0) {}
